...
if not API_TOKEN:
    raise RuntimeError("API_TOKEN doit être défini dans l'environnement (pas de valeur par défaut en dur).")
...
# NOTE: ce montage reste PUBLIC — pas de vérification de token dessus.
# Si les PDF générés sont sensibles, il faudra soit protéger cette route
# différemment (StaticFiles ne supporte pas nativement Depends), soit
# servir les PDF via un endpoint dédié qui vérifie le token avant de
# renvoyer le fichier. Dis-moi si tu veux que je fasse ce changement aussi.
...
# ──────────────────────────────────────────────────────────────────────
# NOTES IMPORTANTES SUR L'AUTHENTIFICATION AJOUTÉE
# ──────────────────────────────────────────────────────────────────────
# - Comme API_TOKEN n'a plus de défaut, le service ne démarrera plus sans
#   la variable d'environnement définie — c'est voulu, mais pense à la
#   définir dans ton .env / config de déploiement avant de relancer.
#
# - Le client (frontend ou script) doit maintenant envoyer l'en-tête
#   Authorization: Bearer <token> sur /api/chat, sinon il recevra un 403
#   (absence d'en-tête, géré par HTTPBearer) ou 401 (token invalide).
#
# - Le montage app.mount("/static", ...) pour les PDF reste public — pas
#   de vérification de token dessus. Si les PDF générés sont sensibles,
#   il faudra soit protéger cette route différemment (StaticFiles ne
#   supporte pas nativement Depends), soit servir les PDF via un endpoint
#   dédié qui vérifie le token avant de renvoyer le fichier. Dis-moi si
#   tu veux que je fasse ce changement aussi.
# ──────────────────────────────────────────────────────────────────────