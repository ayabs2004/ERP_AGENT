...
CHAMPS_TIERS = {
    ...
    "ct_encours_max": {"label": "Encours maximum (DT)", "cle_data": "CT_Encours"},
}
...
ALIAS_CHAMPS = {
    ...
    "sommeil": "ct_validite",
    # ct_encours_max
    "encours": "ct_encours_max", "encours max": "ct_encours_max",
    "encours_max": "ct_encours_max", "plafond": "ct_encours_max",
}
...
async def noeud_modification(state, _parse_mcp_response):
    ...
    if etape == "ATTENTE_CONFIRMATION":
        ...
        if entity_type == "article":
            ...
            if champ_cle == "ct_encours_max":
                try:
                    kwargs["encours_max"] = float(str(nouvelle_val).replace(",", "."))
                except ValueError:
                    kwargs["encours_max"] = 0.0
            elif champ_cle == "ct_validite":
