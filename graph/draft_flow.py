"""
Enrichir la facture depuis le bordereau de livraison (BL) en extraignant des informations (client/fournisseur, date de livraison, référence article, quantité, prix unitaire) à partir de la base de données Sage.
"""
...
def _enrichir_bf_depuis_of(draft: dict) -> dict:
    """
Récupère les informations d'un document de type 'BF' à partir d'un numéro de pièce donné.
"""
    if (draft.get('type_doc') or '').upper() != 'BF':
        return draft
    if draft.get('ref_article'):
        return draft
    num_of = (draft.get('num_of', '') or draft.get('num_piece_source', '') or draft.get('num_piece', '')).strip().upper()
    if not num_of:
        return draft
    conn = _get_conn()
    try:
        row = conn.execute(f'SELECT {sch.C_DL_REF} AS AR_Ref, {sch.C_DL_QTE} AS DL_Qte FROM {sch.T_DOC_LIGNE} WHERE UPPER({sch.C_DL_PIECE}) = ?', (num_of,)).fetchone()
        if row:
...
def _enrichir_nomenclature_preview(draft: dict) -> dict:
    """
Résoudre les informations d'un article de nomenclature pour enrichir le projet de document.
"""
    type_doc = (draft.get('type_doc') or '').upper()
    ...
    try:
        ...
        if not composants:
            return draft
        draft['nomenclature'] = [{'ref': comp['ref_composant'], 'designation': comp['designation'], 'qte': comp['qte_necessaire'] * quantite, 'prix_unitaire': comp['prix_utilise'], 'total': comp['qte_necessaire'] * quantite * comp['prix_utilise']} for comp in composants]
    finally:
        conn.close()
    return draft
...
def _enrichir_prix_preview(draft: dict) -> dict:
    """
Rétourne un objet de prévisualisation enrichi d'informations de prix.
"""
    type_doc = (draft.get('type_doc') or '').upper()
    ...
    try:
        ...
        conn.close()
    return draft
SCHEMAS_DOCUMENTS: dict[str, dict] = {'BL': {'champs': ['code_client', 'ref_article', 'quantite', 'date_livraison'], 'label_champs': {'code_client': 'Quel client ?', 'ref_article': 'Quelle référence article ?', 'quantite': 'Quelle quantité ?', 'date_livraison': 'Quelle date de livraison souhaitée ? (JJ/MM/AAAA)'}}, 'FACTURE': {'champs': ['code_client', 'ref_article', 'quantite', 'date_livraison'], 'label_champs': {'code_client': 'Quel client ?', 'ref_article': 'Quelle référence article ?', 'quantite': 'Quelle quantité ?', 'date_livraison': 'Quelle date de facturation souhaitée ? (JJ/MM/AAAA)'}}, 'BL_ACHAT': {'champs': ['code_fournisseur', 'ref_article', 'quantite', 'prix_unitaire', 'date_livraison'], 'label_champs': {'code_fournisseur': 'Quel fournisseur ?', 'ref_article': 'Quelle référence article ?', 'quantite': 'Quelle quantité reçue ?', 'prix_unitaire': "Quel prix d'achat unitaire ?", 'date_livraison': 'Quelle date de livraison souhaitée ? (JJ/MM/AAAA)'}}, 'FA_ACHAT': {'champs': ['code_fournisseur', 'ref_article', 'quantite', 'prix_unitaire', 'date_livraison'], 'label_champs': {'code_fournisseur': 'Quel fournisseur ?', 'ref_article': 'Quelle référence article ?', 'quantite': 'Quelle quantité ?', 'prix_unitaire': "Quel prix d'achat unitaire ?", 'date_livraison': 'Quelle date de livraison souhaitée ? (JJ/MM/AAAA)'}}, 'OF': {'champs': ['ref_article', 'quantite'], 'label_champs': {'ref_article': 'Quel article fabriquer ?', 'quantite': 'Quelle quantité à produire ?'}}, 'BF': {'champs': ['ref_article', 'quantite', 'num_of'], 'label_champs': {'ref_article': 'Quel article ?', 'quantite': 'Quelle quantité finale produite ?', 'num_of': 'Quel OF associé ?'}}}
_MOTS_CONFIRM_STRICT = {'confirm', 'confirme', 'confirmé', 'confirmer', 'validate', 'valide', 'validé', 'valider'}
_MOTS_ANNULER_STRICT = {'annule', 'annuler', 'annulé', 'stop', 'cancel', 'non merci'}

def est_confirmation_stricte(texte: str) -> bool:
    """
Fonction qui vérifie si un texte est une confirmation stricte en comparant sa forme nettoyée à une liste de mots de confirmation.
"""
    return texte.strip().lower().rstrip('!.') in _MOTS_CONFIRM_STRICT

def est_annulation_stricte(texte: str) -> bool:
    """
Vérifie si le texte d'une commande est une annulation stricte.
"""
...
def question_pour_champ(type_doc: str, champ: str, draft: dict | None=None) -> str:
    """
Rôle de la fonction : Créer une question à afficher pour un champ spécifique d'un document.
"""
    schema = SCHEMAS_DOCUMENTS.get((type_doc or '').upper(), {})
...
async def _verifier_stock_draft(draft: dict) -> tuple[bool, str]:
    """
Vérifie si le stock d'un article est suffisant pour un draft.
"""
    if draft.get('type_doc', '').upper() != 'BL' or not draft.get('ref_article') or float(draft.get('quantite', 0)) <= 0:
        return (True, '')
    try:
        raw = await mcp_pool.call('nl2sql', 'verifier_stock_article', {'ref_article': draft['ref_article']})
        if not raw or raw.startswith('STOCK : NON_TROUVE'):
            return (True, '')
        m = re.search('net\\s*:\\s*([+-]?\\d+(?:[.,]\\d+)?)', raw, re.IGNORECASE)
        if m:
            qte_net = float(m.group(1).replace(',', '.'))
            qte_demandee = float(draft.get('quantite', 0) or 0)
            if qte_net < qte_demandee:
                return (False, f"❌ Stock insuffisant pour '{draft['ref_article']}' : {qte_net:.0f} net disponible pour {qte_demandee:.0f} demandées.")
    except Exception as e:
...
def injecter_reponse_dans_draft(type_doc: str, draft: dict, texte_user: str) -> dict:
    """
Cette fonction injecte les informations saisies par l'utilisateur dans un document de type spécifique en fonction des champs qui manquent encore.
"""
    ...
    if champ == 'quantite':
        ...
        m = re.search('\\b(OF[A-Z0-9]+)\\b', texte, re.IGNORECASE)
...
def construire_draft_depuis_state(state: dict) -> dict:
    """
Crée un draft en fonction de l'état fourni.
"""
    type_doc = (state.get('type_doc') or 'BL').upper()
    draft = {'type_doc': type_doc, 'ref_article': state.get('ref_article', ''), 'quantite': state.get('quantite', 0.0) or 0.0, 'date_str': datetime.now().strftime('%d/%m/%Y')}
    if type_doc == 'BL_ACHAT':
        draft['code_fournisseur'] = state.get('code_client', '')
    elif type_doc == 'BF':
        draft['code_client'] = state.get('code_client', '') or 'PROD-INT'
        draft['num_of'] = state.get('num_piece', '') or state.get('num_piece_source', '') or state.get('dernier_num_piece', '')
    elif type_doc == 'OF':
        draft['code_client'] = state.get('code_client', '') or 'PROD-INT'
...
async def generer_preview(draft: dict) -> tuple[str, str]:
    """
Renvoi un aperçu sous forme de texte brut et sous forme de PDF d'un document.
"""
    ...
    if draft.get('nomenclature'):
        ...
        for comp in draft['nomenclature']:
            lignes.append(f"    - {comp.get('ref', '?')} {comp.get('designation', '').strip()} x{comp.get('qte', 0):g}" + (f" @ {comp.get('prix_unitaire', 0):.3f}" if comp.get('prix_unitaire') else ''))
    lignes.append('─' * 50)
...
async def executer_draft_confirme(draft: dict, mcp_workflow_bl, mcp_workflow_of, mcp_workflow_bf, mcp_workflow_bl_achat, mcp_pool_transformer_document=None, mcp_workflow_facture=None, mcp_workflow_fa_achat=None) -> dict:
    """
Cette fonction permet d'exécuter une procédure de confirmation de draft (modèle) selon le type de document spécifié.
"""
    type_doc = (draft.get('type_doc') or '').upper()
    if type_doc == 'BL':
    ...
    if type_doc == 'OF':
        return await mcp_workflow_of(draft.get('ref_article', ''), float(draft.get('quantite', 0)), draft.get('code_client', 'PROD-INT'))
    if type_doc == 'BF':
        return await mcp_workflow_bf(draft.get('ref_article', ''), float(draft.get('quantite', 0)), draft.get('num_of', ''), draft.get('code_client', 'PROD-INT'))
...
async def generer_pdf_final(draft: dict, num_piece: str) -> str:
    """
Fonction asynchrone qui génère un PDF final à partir d'un jeton de draft et d'un numéro de pièce.
"""
...
def ajouter_alerte_bf_requis(contexte_session: dict, num_of: str, ref_article: str, qte_prevue: float):
    """
Fonction pour ajouter une alerte persistante en cas de manque de Bon de Fabrication (BF) requis pour un certain OF.
"""
    alertes = contexte_session.setdefault('alertes_persistantes', [])
    if any((a.get('num_of') == num_of for a in alertes)):
        return
    alertes.append({'id': f'BF_{num_of}', 'type': 'BF_REQUIS', 'num_of': num_of, 'ref_article': ref_article, 'qte_prevue': qte_prevue, 'message': f"""⚠️  L'OF {num_of} ({ref_article}, {qte_prevue:g} u prévues) n'a pas encore de Bon de Fabrication associé. Tapez "crée le BF pour {num_of}" pour le finaliser.""", 'ts': time.time()})

async def enrichir_draft(draft: dict, mcp_pool) -> dict:
    """
Fonction qui enrichit un dossier de commande de vente en récupérant les informations d'un client et d'un fournisseur à partir d'un pool de connexion.
"""
    import json
    ...
    if code_fourn and (not draft.get('intitule_fournisseur')):
        try:
            txt = await mcp_pool.call('nl2sql', 'executer_sql_vanna', {'sql': f"SELECT TOP 1 CT_Intitule FROM F_COMPTET WHERE CT_Num='{code_fourn}'", 'description': f'Intitulé fournisseur {code_fourn}'})
            if not txt:
    ...
    if ref_article and (not draft.get('prix_unitaire')):
        try:
            col_prix = 'AR_PrixAch' if type_doc in ('BL_ACHAT', 'FA_ACHAT') else 'AR_PrixVen'
            txt = await mcp_pool.call('nl2sql', 'executer_sql_vanna', {'sql': f"SELECT TOP 1 AR_Design, {col_prix} FROM F_ARTICLE WHERE UPPER(AR_Ref)=UPPER('{ref_article}')", 'description': f'Prix et désignation de {ref_article}'})
            if not txt:
...
def resoudre_alerte_bf(contexte_session: dict, num_of: str):
    """
Supprime une alerte persistante en fonction de son numéro d'objet de fabrication.
"""
    alertes = contexte_session.get('alertes_persistantes', [])
    contexte_session['alertes_persistantes'] = [a for a in alertes if a.get('num_of') != num_of]

def formater_alertes_persistantes(contexte_session: dict) -> str:
    """
Cette fonction génère une chaîne de caractères contenant une liste d'alertes persistantes.
"""
    alertes = contexte_session.get('alertes_persistantes', [])
    if not alertes:
        return ''
