"""reviser_candidats.py

Provides a command‑line tool to manually review candidate learning examples
generated by the semi‑automatic learning pipeline. It lists candidates
filtered by a configurable minimum number of unchallenged occurrences,
prompts the user to accept, reject or skip each candidate, updates the
candidate file accordingly and inserts accepted examples into
semantic_examples.yaml via the apprentissage_semi_auto utilities.
"""
...
def _lister_tous(min_occurrences: int) -> list[dict]:
    """Aggregate raw candidate entries from the JSONL file, deduplicate them
    by question/action, compute total occurrences and best score, and return
    a list of candidates whose occurrence count meets the given threshold."""
...
async def _main_async(min_occurrences: int):
    """Asynchronously load the semantic classifier, retrieve candidates via
    _lister_tous, interactively prompt the user to validate each candidate,
    update the candidate file to keep only skipped entries, and forward
    accepted candidates to the validation/insertion routine."""
...
def main():
    """Parse command‑line arguments and run the asynchronous review workflow."""
